...
class MaskedLinear(nn.Module):
    ...
    def forward(self, input_, context=None):
        output = F.linear(input_, self.mask * self.linear.weight,
                          self.linear.bias)
        if context is None:
            return output
        else:
...
class MADE(nn.Module):
    ...
    This is used to build MAF: Masked Autoregressive Flow
    ...
    def __init__(self, num_input, num_output, num_hidden, num_context):
        ...
        self._check_masks()
        modules = []
        self.input_context_net = MaskedLinear(num_input, num_hidden,
                                              self._masks[0], num_context)
    ...
    def _build_masks(self, num_input, num_output, num_hidden, num_layers):
        ...
        for i in range(1, num_layers + 1):
            ...
            if i == num_layers:
                ...
                # self._m.append(np.arange(1, num_hidden + 1) % \
                # (num_input - 1) + 1)
            if i == num_layers:
    ...
    def _check_masks(self):
        """Check that the connectivity matrix between layers
        is lower triangular."""
