...
Notes:
  ...
  to download binary MNIST file
  - batch size is the innermost dimension, then the sample dimension,
  then latent dimension
"""
...
class Model(nn.Module):
    """Bernoulli model parameterized by a generative network with
    Gaussian latents for MNIST."""
    ...
    def forward(self, z, x):
        ...
        log_p_z = self.log_p_z(self.p_z_loc, self.p_z_scale, z).sum(
            -1, keepdim=True)
...
class NormalLogProb(nn.Module):
    ...
    def forward(self, loc, scale, z):
        ...
        return -0.5 * torch.log(2 * np.pi * var) - \
            torch.pow(z - loc, 2) / (2 * var)
...
def load_binary_mnist(cfg, **kwcfg):
    ...
    f = h5py.File(pathlib.os.path.join(pathlib.os.environ['DAT'],
                  'binarized_mnist.hdf5'), 'r')
    x_train = f['train'][::]
    x_val = f['valid'][::]
    x_test = f['test'][::]
...
if __name__ == '__main__':
    ...
    for step, batch in enumerate(cycle(train_data)):
        ...
        if step % cfg.log_interval == 0:
            print(f'step:\t{step}\ttrain elbo: ' +
                  f'{elbo.detach().cpu().numpy().mean():.2f}')
            with torch.no_grad():
                valid_elbo, valid_log_p_x = evaluate(
                    cfg.n_samples, model, variational, valid_data)
            print(f'step:\t{step}\t\tvalid elbo: ' +
                  f'{valid_elbo:.2f}\tvalid log p(x): {valid_log_p_x:.2f}')
            ...
            if num_no_improvement > cfg.early_stopping_interval:
                ...
                print(f'step:\t{step}\t\ttest elbo: ' +
                      f'{test_elbo:.2f}\ttest log p(x): {test_log_p_x:.2f}')
                break
